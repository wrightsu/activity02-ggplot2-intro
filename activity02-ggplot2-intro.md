...
**Response**: Species and island seem to be the biggest contributor. Sex
...
**Response**: As body mass goes up, so does flipper length.
...
**Response**: Gentoos have large body mass and bigger flipperrs. Adelie
and chinstraps are similarly sized, though adelies hit the smaller end
of the range.
...
``` r
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = body_mass_g)) +
  geom_point(mapping = aes(alpha = flipper_length_mm, size = body_mass_g ))
```
...
**Response**: Gives a visual respresentation of size of animal and
allows data points to more easily been seen.
